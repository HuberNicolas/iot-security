...
const char text1[100]={"25061944"}; // the client's identity changed to studi
const char hash[32]; // used to store hash
int key = 4; // key for Caesar cipher algorithm
...
void receive_hash()
{
 ...
 if(reply==1){
    uip_udp_packet_send(server_conn,"ACK",sizeof("ACK")); //send ACK message to the client no need to change here
...
PRINTF("Server sending reply....\n");
    ...
    uip_udp_packet_send(server_conn,"Message Received",sizeof("Message Received")); //sending UDP packets through uIP changed from Msg Received
